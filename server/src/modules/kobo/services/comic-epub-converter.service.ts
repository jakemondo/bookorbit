...
async function extractCb7Pages(absolutePath: string): Promise<Page[]> {
  ...
  try { sz.FS.mkdir(outDir); } catch { /* exists */ }
  ...
  // Cleanup WASM VFS
  try { sz.FS.unlink(archPath); } catch { /* ok */ }
  for (const name of pageNames) { try { sz.FS.unlink(`${outDir}/${name}`); } catch { /* ok */ } }
  try { sz.FS.rmdir(outDir); } catch { /* ok */ }
